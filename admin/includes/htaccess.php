...
function sixscan_htaccess_install( $htaccess_sixscan_version = "") {
	...
	try { 
		...
		if ( ! $wp_filesystem->copy( SIXSCAN_HTACCESS_6SCAN_GATE_SOURCE, SIXSCAN_HTACCESS_6SCAN_GATE_DEST ) ) {
			throw new Exception( 'Failed to find ' . SIXSCAN_HTACCESS_6SCAN_GATE_FILE_NAME . ' during installation' );
		}
		
		if ( ! $wp_filesystem->chmod( SIXSCAN_HTACCESS_6SCAN_GATE_DEST, 0755 ) ) {
			throw new Exception( 'Failed to chmod ' . SIXSCAN_HTACCESS_6SCAN_GATE_FILE_NAME . ' during installation' );
		}
	
		if ( ! $wp_filesystem->copy( SIXSCAN_SIGNATURE_SRC, SIXSCAN_SIGNATURE_DEST ) ) {
